...
document.getElementById('posSearchWa').addEventListener('input', ()=>{
  // live-search as the cashier types a name
  if(document.getElementById('posSearchWa').value.trim().length>=2) doPosSearch();
});
...
function selectPosMember(member){
  ...
  document.getElementById('posSearchResult').innerHTML =
    `<div class="banner banner-good">Member dipilih: ${member.name} · ${member.tier}</div>`;
  renderPosMemberCard();
  recalcPos();
}

function doPosSearch(){
  const query = document.getElementById('posSearchWa').value.trim();
  ...
  if(matches.length > 1){
    posMember = null;
    resultEl.innerHTML = `
      <div class="banner banner-warn">${matches.length} member ditemukan, pilih salah satu:</div>
      <div id="posSearchMatches"></div>`;
    const listEl = document.getElementById('posSearchMatches');
    listEl.innerHTML = matches.map(m=>`
      <div class="reward-opt" data-id="${m.id}">
        <div>
          <div class="rname">${m.name}</div>
          <div class="rmeta">${m.whatsapp} · ${m.tier}</div>
        </div>
        <div class="rval">${m.availablePoints.toLocaleString('id-ID')} Pts</div>
      </div>`).join('');
    listEl.querySelectorAll('.reward-opt').forEach(el=>{
      el.addEventListener('click', ()=> selectPosMember(state.members.find(m=>m.id===el.dataset.id)));
    });
    renderPosMemberCard();
    recalcPos();
    return;
  ...
  // no name match — offer to search/register by WhatsApp instead
  posMember = null;
  const wa = normalizeWa(query);
  const byWa = wa ? state.members.find(m=>m.whatsapp===wa) : null;
  if(byWa){
    selectPosMember(byWa);
    return;
  }
  resultEl.innerHTML = `
    <div class="banner banner-warn">Tidak ada member dengan nama "${query}".</div>
  ...
  document.getElementById('quickRegisterBtn').addEventListener('click', ()=>{
    const wa2 = prompt('Nomor WhatsApp member baru:');
    if(!wa2) return;
    const waNorm = normalizeWa(wa2);
    if(state.members.find(m=>m.whatsapp===waNorm)){ alert('Nomor WhatsApp sudah terdaftar.'); return; }
    posMember = registerMember(query, waNorm);
